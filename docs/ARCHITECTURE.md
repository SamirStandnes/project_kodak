...
The ingestion process is automated by `kodak/pipeline/ingest.py`.

### Step 1: Parser Routing
The system scans `data/new_raw_transactions/`. If a file is found in `nordnet/`, it dynamically loads `kodak/pipeline/parsers/nordnet.py`.
...
This is the core "Brain" of the system, located in `kodak/shared/calculations.py`.
...
The system supports any asset that Yahoo Finance can price:

| Asset Class | ISIN | Symbol Format | Example |
|-------------|------|---------------|---------|
| Stocks | Required | Exchange suffix | `AAPL`, `NHY.OL`, `7203.T` |
| ETFs | Required | Exchange suffix | `SPY`, `IBIT` |
| Crypto | None | `XXX-USD` | `BTC-USD`, `ETH-USD` |
| Funds | Required | Exchange suffix | Varies |

### 5.1 Traditional Securities (Stocks, ETFs, Funds)

Brokers provide an **ISIN** (International Securities Identification Number). The `isin_map.csv` file maps ISINs to Yahoo Finance symbols:

```csv
isin,symbol,currency,sector,region,country,asset_class
US0378331005,AAPL,USD,Technology,Americas,United States,Equity
```

### 5.2 Cryptocurrencies

Crypto assets don't have ISINs. Parsers for crypto exchanges should:

1. Set `isin` to `None`
2. Set `symbol` directly using Yahoo Finance format (`BTC-USD`, `ETH-USD`)
3. The `isin_map.csv` is not used for crypto

**Example parser snippet:**
```python
txn = create_empty_transaction()
txn['isin'] = None
txn['symbol'] = 'BTC-USD'  # Yahoo Finance format
txn['currency'] = 'USD'
txn['quantity'] = 0.5
txn['price'] = 42000.00
```

### 5.3 Adding New Asset Classes

Any asset with a Yahoo Finance ticker can be tracked. The key requirements:
- `symbol` must match Yahoo Finance exactly
- `currency` must be the asset's trading currency
- Prices are fetched automatically via `kodak/pipeline/fetch_prices.py`
