...
def enrich_staging_data():
    ...
    with get_db_connection() as conn:
        ...
        updates_tx = []
        if not df_tx.empty:
            logger.info(f"Enriching {len(df_tx)} transactions with historical FX rates...")
            ...
            if updates_tx:
                ...
                conn.commit()
                logger.info(f"Updated {len(updates_tx)} transactions with historical rates.")
        ...
        updates_fee = []
        if not df_fee.empty:
            logger.info(f"Enriching {len(df_fee)} fees with historical FX rates...")
            ...
            if updates_fee:
                ...
                conn.commit()
                logger.info(f"Updated {len(updates_fee)} fees with historical rates.")
...
def _get_rate(currency, date_str, cache):
    ...
    logger.debug(f"Fetching {pair} for {date_str}...")
    try:
