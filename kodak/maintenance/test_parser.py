...
def test_parser(parser_name, file_path):
    ...
    # 1. Import
    try:
        module_name = f"kodak.pipeline.parsers.{parser_name}"
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python -m kodak.maintenance.test_parser <parser_name> <sample_file_path>")
        print("Example: python -m kodak.maintenance.test_parser nordnet data/new_raw_transactions/nordnet/sample.csv")
    else:
